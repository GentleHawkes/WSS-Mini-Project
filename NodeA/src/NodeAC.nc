...
implementation {
	...
	uint16_t ackDataPacketCounter = 0;
	...
	event void DataSnd.sendDone(message_t *msg, error_t error){
		...
		if (call packAck.wasAcked(msg)) {
			call Leds.led1Toggle();
			ackDataPacketCounter++;
		}
		busy = FALSE;
		if (dataPacketCounter % 100 == 0) {
			printf("Packets sent: %d\nPackets lost: %d\n\n", dataPacketCounter, dataPacketCounter - ackDataPacketCounter);
			printfflush();
			if (dataPacketCounter == 1000) {
				call TimerData.stop();
				printf("Lost packets: %d\nHop counter: %d\n\n", 1000 - ackDataPacketCounter, hopCounter);
				printfflush();
			} else {
				call TimerData.stop();
				call TimerProbe.startPeriodic(SEND_PROBE_INTER_MS);
